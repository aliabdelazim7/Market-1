...
async function fixAnomalies() {
  ...
  console.log("Updating purchase invoices...");
  for (const id of purchaseIds) {
    ...
    const { data: updateRes, error: updateErr } = await supabase
      .from('purchase_invoices')
      .update({
        paid_cash: fetchRes.paid_amount,
        paid_wallet: 0
      })
      .eq('id', id)
      .select();
    ...
    if (updateErr) {
      ...
      console.log(`Successfully updated purchase invoice ${fetchRes.invoice_number} (ID: ${id})`);
    }
  }
  ...
  for (const id of expenseIds) {
    ...
    if (updateErr) {
      ...
      console.log(`Successfully updated expense (ID: ${id}) with note "${fetchRes.note}"`);
    }
  }
