...
const inputClass = 'w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 font-bold focus:outline-none focus:ring-2 focus:ring-indigo-100';
...
export default function Financing() {
  ...
  const renderAccountCard = (account: FinancingAccount) => {
    ...
    const iconClass = account.type === 'association'
      ? 'p-3 rounded-2xl bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400'
      : 'p-3 rounded-2xl bg-indigo-50 text-indigo-600';
    return (
      <button
        key={account.id}
        onClick={() => setSelectedId(account.id)}
        className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 text-right shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition"
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-lg font-black text-slate-800 dark:text-slate-100">{account.lender_name}</p>
              <p className="text-xs font-bold text-slate-400 mt-1">{account.description || account.lender_details || 'بدون وصف'}</p>
            </div>
          </div>
          <span className={`px-2.5 py-1 rounded-lg text-[11px] font-black ${account.status === 'closed' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300' : 'bg-amber-100 text-amber-700'}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-5">
          <div className="bg-emerald-50 dark:bg-emerald-500/10 rounded-xl p-3">
            <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-400">التحصيل</p>
            <p className="font-black text-emerald-800 dark:text-emerald-300 mt-1">{money(account.collection_amount, storeSettings.currency)}</p>
          </div>
          <div className="bg-red-50 dark:bg-red-500/10 rounded-xl p-3">
            <p className="text-[10px] font-black text-red-600 dark:text-red-400">المتبقي</p>
            <p className="font-black text-red-800 dark:text-red-300 mt-1">{money(summary.remaining, storeSettings.currency)}</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-3">
            <p className="text-[10px] font-black text-slate-500 dark:text-slate-400">الدفعات</p>
            <p className="font-black text-slate-800 dark:text-slate-100 mt-1">{summary.repayments.filter((p) => p.status === 'paid').length}/{summary.repayments.length}</p>
          </div>
        </div>
  ...
  return (
    <div className="p-4 md:p-8 space-y-6" dir="rtl">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-800 dark:text-slate-100">السلف والجمعيات</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">ملفات مستقلة لكل سلفة أو جمعية، مع تحصيل وسداد جزئي وسجل معاملات واضح.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/30 p-5 rounded-2xl">
          <p className="text-xs font-black text-emerald-700 dark:text-emerald-300">دخل للخزنة من التمويل</p>
          <h3 className="text-2xl font-black text-emerald-800 dark:text-emerald-300 mt-2">{money(stats.collected, storeSettings.currency)}</h3>
        </div>
        <div className="bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/30 p-5 rounded-2xl">
          <p className="text-xs font-black text-red-700 dark:text-red-300">سداد خرج من الخزنة</p>
          <h3 className="text-2xl font-black text-red-800 dark:text-red-300 mt-2">{money(stats.repaid, storeSettings.currency)}</h3>
        </div>
        <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/30 p-5 rounded-2xl">
          <p className="text-xs font-black text-amber-700 dark:text-amber-300">إجمالي المتبقي للسداد</p>
          <h3 className="text-2xl font-black text-amber-800 dark:text-amber-300 mt-2">{money(stats.remaining, storeSettings.currency)}</h3>
        </div>
      </div>

      <div className="flex flex-col lg:flex-row lg:items-center gap-3">
        <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-2 flex gap-2 w-fit shadow-sm">
          <button
            onClick={() => setActiveType('loan')}
            className={`px-5 py-3 rounded-xl font-black transition ${activeType === 'loan' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
          >
            السلف ({grouped.loan.length})
          </button>
          <button
            onClick={() => setActiveType('association')}
            className={`px-5 py-3 rounded-xl font-black transition ${activeType === 'association' ? 'bg-violet-600 text-white shadow' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
          >
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-2 flex gap-2 w-fit shadow-sm">
          {[
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl font-black text-sm transition ${statusFilter === item.value ? 'bg-slate-900 dark:bg-slate-700 text-white shadow' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
            >
      ...
      {selectedAccount && selectedSummary && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[92vh] overflow-y-auto">
            <div className="bg-white dark:bg-slate-800 p-5 rounded-t-2xl border-b border-slate-100 dark:border-slate-800 flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100">{selectedAccount.lender_name}</h2>
                </div>
                <div className="flex flex-wrap gap-3 mt-3 text-xs font-bold text-slate-500 dark:text-slate-400">
              ...
              <button onClick={() => setSelectedId(null)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-red-500">
                <X size={22} />
              </button>
            </div>
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                <p className="text-[11px] font-black text-emerald-600 dark:text-emerald-400">مبلغ التحصيل</p>
                <p className="text-xl font-black text-emerald-800 dark:text-emerald-300 mt-2">{money(selectedAccount.collection_amount, storeSettings.currency)}</p>
              </div>
              <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                <p className="text-[11px] font-black text-red-600 dark:text-red-400">المتبقي للسداد</p>
                <p className="text-xl font-black text-red-800 dark:text-red-300 mt-2">{money(selectedSummary.remaining, storeSettings.currency)}</p>
              </div>
              <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                <p className="text-[11px] font-black text-slate-500 dark:text-slate-400">مدفوع من السداد</p>
                <p className="text-xl font-black text-slate-800 dark:text-slate-100 mt-2">{money(selectedSummary.paid, storeSettings.currency)}</p>
              </div>
              <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                <p className="text-[11px] font-black text-slate-500 dark:text-slate-400">الحالة</p>
                <p className="text-xl font-black text-slate-800 dark:text-slate-100 mt-2">{selectedAccount.status === 'closed' ? 'مغلق' : 'مفتوح'}</p>
              </div>
            </div>
            ...
            <div className="p-5 grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-5">
              <div className="space-y-4">
                <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                  <h3 className="font-black text-slate-800 dark:text-slate-100 mb-4">التحصيل</h3>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                  <h3 className="font-black text-slate-800 dark:text-slate-100 mb-4">دفعات السداد</h3>
              ...
              <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                <h3 className="font-black text-slate-800 dark:text-slate-100 mb-4">معاملات الملف</h3>
                {selectedTransactions.length === 0 ? (
                  <p className="text-center text-slate-400 font-bold py-10">لا توجد معاملات مدفوعة بعد.</p>
                ) : (
                  <div className="space-y-3">
                    {selectedTransactions.map((tx) => (
                      <div key={tx.id} className="border border-slate-100 dark:border-slate-800 rounded-xl p-3 flex items-start justify-between gap-3">
                        <div>
                          <p className={`font-black ${tx.transaction_type === 'collection' ? 'text-emerald-700 dark:text-emerald-300' : 'text-red-700'}`}>
                        ...
                        <span className="text-[11px] font-black text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 px-2 py-1 rounded-lg">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] overflow-y-auto">
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <h2 className="text-xl font-black text-slate-800 dark:text-slate-100">إضافة {form.type === 'association' ? 'جمعية' : 'سلفة'}</h2>
              <button onClick={() => setShowForm(false)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-red-500">
                <X size={20} />
              </button>
            </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField label="مبلغ التحصيل">
                    <input type="number" value={form.collection_amount} onChange={(e) => setForm({ ...form, collection_amount: e.target.value })} className={`${inputClass} text-emerald-700 dark:text-emerald-300 font-black`} />
                  </FormField>
                  <FormField label="إجمالي السداد">
                    <input type="number" value={form.principal_amount} onChange={(e) => setForm({ ...form, principal_amount: e.target.value })} className={`${inputClass} text-red-700 dark:text-red-300 font-black`} />
                  </FormField>
                ...
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <p className="font-black text-slate-700 dark:text-slate-200">جدول الدفعات</p>
                    <p className="text-xs font-bold text-slate-400">عدلي التاريخ أو المبلغ قبل الحفظ</p>
                  </div>
                  <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                    {repaymentRows.map((payment, index) => (
                      <div key={index} className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl p-3 grid grid-cols-[auto_1fr_1fr] gap-2 items-center">
            ...
            <div className="p-5 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
              <button onClick={() => setShowForm(false)} className="px-5 py-3 rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-black">إلغاء</button>
      ...
      {paymentTarget && (
        <div className="fixed inset-0 bg-slate-900/50 z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto p-5">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-black text-slate-800 dark:text-slate-100">
                {paymentTarget.payment_type === 'collection' ? 'تحصيل' : 'سداد'} جزئي / كامل
              </h2>
              <button onClick={() => setPaymentTarget(null)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-red-500">
                <X size={20} />
              </button>
            </div>
            <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 mt-4">
              <p className="text-xs font-black text-slate-500 dark:text-slate-400">المتبقي على الدفعة</p>
              <p className="text-2xl font-black text-slate-800 dark:text-slate-100 mt-1">{money(getRemaining(paymentTarget), storeSettings.currency)}</p>
            </div>
  ...
  return (
    <div className="border border-slate-100 dark:border-slate-800 rounded-2xl p-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex items-start gap-3">
          <div className={`p-2 rounded-xl ${isCollection ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-red-50 text-red-600'}`}>
            {isCollection ? <Banknote size={18} /> : <ReceiptText size={18} />}
          </div>
          <div>
            <p className="font-black text-slate-800 dark:text-slate-100">{isCollection ? 'تحصيل' : payment.note || 'دفعة سداد'}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-200 px-3 py-2 rounded-xl text-xs font-black">الإجمالي {money(payment.amount, currency)}</span>
          <span className="bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 px-3 py-2 rounded-xl text-xs font-black">مدفوع {money(paid, currency)}</span>
          <span className="bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-300 px-3 py-2 rounded-xl text-xs font-black">باقي {money(remaining, currency)}</span>
          {!isPaid && (
          ...
          {isPaid && <span className="bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 px-3 py-2 rounded-xl text-xs font-black flex items-center gap-1"><CheckCircle2 size={14} /> مكتمل</span>}
        </div>
      </div>
      <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full mt-4 overflow-hidden">
